...
## Phase 0.5 — Locate the vendored build code

**Goal:** Confirm the orchestration plumbing is present. In this boilerplate the
...
- `dashboard-template/` — Obsidian plugin (Preact + esbuild) — used in Phase 11
...
cp _workspace/dashboard/metric-scripts/_common.py ~/.claude/skills/metrics-pull/scripts/
cp _workspace/dashboard/metric-scripts/run_all.ps1 ~/.claude/skills/metrics-pull/scripts/    # Windows
cp _workspace/dashboard/metric-scripts/run_all.sh ~/.claude/skills/metrics-pull/scripts/     # macOS / Linux
...
```bash
cp _workspace/dashboard/metric-scripts/pull_template_api.py ~/.claude/skills/metrics-pull/scripts/
cp _workspace/dashboard/metric-scripts/pull_template_scrape.py ~/.claude/skills/metrics-pull/scripts/
cp _workspace/dashboard/metric-scripts/pull_template_local.py ~/.claude/skills/metrics-pull/scripts/
```
...
cp _workspace/dashboard/hooks/activity-log.js ~/.claude/hooks/
cp _workspace/dashboard/hooks/package.json ~/.claude/hooks/
```
...
> [ACTION] The dashboard template is vendored at `_workspace/dashboard/dashboard-template/` in this repo — no clone or pull needed. Confirm it:
>
> ```bash
> ls _workspace/dashboard/dashboard-template/
> ```
>
...
> [ACTION] The plugin is built in a separate dir (`~/projects/my-dashboard`, from Phase 3) while the template lives in this boilerplate. Set `COMIND_REPO` to this repo's absolute path so the copies resolve. Wipe your Phase 3 scaffold's `src/` so the template lands clean, then copy template files in:
>
> ```bash
> COMIND_REPO="$(pwd)"   # run from the boilerplate repo root BEFORE cd-ing away
> cd ~/projects/my-dashboard
> rm -rf src
...
> cp "$COMIND_REPO"/_workspace/dashboard/dashboard-template/package.json.template ./package.json
> cp "$COMIND_REPO"/_workspace/dashboard/dashboard-template/esbuild.config.mjs.template ./esbuild.config.mjs
> ```
