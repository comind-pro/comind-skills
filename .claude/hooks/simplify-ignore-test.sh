...
# Run: bash .claude/hooks/simplify-ignore-test.sh
...
eval "$(sed -n '/^filter_file()/,/^}/p' "$SCRIPT_DIR/simplify-ignore.sh")"
...
warning_out=$(echo 'NOT_JSON{{{' | bash "$SCRIPT_DIR/simplify-ignore.sh" 2>&1) || true
