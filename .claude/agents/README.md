...
This is the only orchestration pattern this repo endorses. See [_workspace/references/orchestration-patterns.md](../../_workspace/references/orchestration-patterns.md) for the full pattern catalog and anti-patterns.
...
Subagents only report results back to the main agent. Agent Teams let teammates message each other directly. Use subagents when reports are enough; use Agent Teams when sub-agents need to challenge each other's findings (e.g. competing-hypothesis debugging). See [_workspace/references/orchestration-patterns.md](../../_workspace/references/orchestration-patterns.md) for the full mapping.
...
5. If the persona enables a new orchestration pattern, document it in `_workspace/references/orchestration-patterns.md` rather than inventing the pattern in the persona file itself.
