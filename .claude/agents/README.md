...
This is the only orchestration pattern this repo endorses. See [_workspace/docs/references/orchestration-patterns.md](../../_workspace/docs/references/orchestration-patterns.md) for the full pattern catalog and anti-patterns.
...
- Replicates work that slash commands and skill `description` triggers already do
...
Subagents only report results back to the main agent. Agent Teams let teammates message each other directly. Use subagents when reports are enough; use Agent Teams when sub-agents need to challenge each other's findings (e.g. competing-hypothesis debugging). See [_workspace/docs/references/orchestration-patterns.md](../../_workspace/docs/references/orchestration-patterns.md) for the full mapping.
...
1. Create `.claude/agents/<role>.md` with the same frontmatter format used by existing personas.
...
5. If the persona enables a new orchestration pattern, document it in `_workspace/docs/references/orchestration-patterns.md` rather than inventing the pattern in the persona file itself.
