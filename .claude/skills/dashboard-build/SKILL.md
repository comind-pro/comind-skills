...
description: Builds a personal comind-dashboard observability dashboard — an Obsidian-native plugin backed by a markdown vault, a queue-watching runner daemon, metric-pull scripts, and an activity-log hook. Use when a user wants a dashboard for their Claude Code skills, asks to "build the dashboard" / "build my comind-dashboard" / "set up the dashboard", or as Part 3 of the comind-dashboard build (after skills in Part 1). Drives an 11-phase build with a customization interview and per-phase verify/fix gates. Installs from the vendored `_workspace/dashboard/` templates in this repo.
---
...
This skill builds the observability layer of comind-dashboard: a dashboard
pane inside Obsidian that surfaces the metrics and one-click
...
It is **Part 3** of the comind-dashboard build. Part 1 (`skill-architecture`) builds
...
- The user invokes: "build the dashboard", "build my comind-dashboard", "set up the
...
The build code is **vendored in this repo at `_workspace/dashboard/`** — everything
ships with the template, nothing to clone. It holds:

- `_workspace/dashboard/runner/` — the queue runner daemon
- `_workspace/dashboard/hooks/` — the activity-log hook
- `_workspace/dashboard/metric-scripts/` — metric-pull scripts
- `_workspace/dashboard/dashboard-template/` — the Obsidian plugin (Preact + esbuild)

The full phase-by-phase guide — every `[ACTION]`, `[VERIFY]`, `[FIX]`
...
|---|---|
| "Skip the Phase 0 interview, use defaults" | The interview fills the `$VARIABLES` every later phase reads. Skipping it means hardcoding someone else's domain defaults. Run it. |
